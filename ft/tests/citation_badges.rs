//! Integration tests for citation badges on `ft notes recent`:
//! `cited:` / `cited*:` badge lines, the `cited_in`
...
/// Find the feed row whose `section` contains `needle`.
...
#[test]
fn history_json_and_uncited_carry_citation_state() {
    let tmp = make_vault();
    let out = ft()
