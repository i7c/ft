...
- [x] 9.4 / 9.5 — first shared headless handler landed. `tasks.complete-by-id` (new `CommandDef` with required `id` arg, declared in `TASKS_COMMANDS`) dispatches via `cmd/do.rs::handle_tasks_complete_by_id`, which calls `ft_core::selector::resolve(Selector::Id(...))` + `ft_core::task::ops::complete_task` — the same paths the CLI's `ft tasks complete` and the TUI's `tasks.complete` use. Validated end-to-end against a temp vault. Other registry commands without a headless handler still exit 3 with a clear message; add new arms as more atomic ops become callable without TUI ambient state.
- [x] 9.6 `--format text|json` flag accepted (default `text`). Both paths are exercised by `tasks.complete-by-id`: text emits `Completed <file>:<line>\n  <serialized>` (plus a `Recurring:` line for recurring tasks); json emits `{"command":"tasks.complete-by-id","outcome":"ok","details":{"file","line","serialized",("next_instance":…)}}`. Top-level `--json-errors` is honoured by `main.rs`'s error-output path (unchanged).
- [x] 9.7 12 unit tests cover: parse-args (empty, sorted, missing-equals rejection, value-with-equals), validate-args (pass + fail-on-missing), run (unknown command, modal-opening rejection, missing-arg rejection, no-handler rejection for `app.switch-tab`), and end-to-end completion via `tasks.complete-by-id` (success path against a temp vault + clear error on unknown id).

## 10. Status-bar modal hint

- [x] 10.1 `CommandDef.is_primary: bool` confirmed in `confirm_def`/`cancel_def` and explicit on the per-modal toggle/commit/leader verbs (`section-move.toggle`, `related.{toggle,commit}`, `move.use-selected-as-{source,target}`, `periodic-leader.{daily,weekly}`, `capture-var.next-or-commit`, `query-bar.apply`, `rename.commit`). All 12 modals export at least one primary chord.
- [x] 10.2 `help::modal_primary_hints(&KeyMap, &CommandRegistry) -> Vec<(String, String)>` returns up to three `(chord_display, short_label)` pairs (label = verb portion after the last `.`). `App::draw` calls it when a modal is active and passes the result into `StatusBarState::modal_hints`. The center cell renders the hints in `chord:label` form (magenta chord, dim ":", white label) when no toast is present; the right cell keeps `modal: <name>` from extract-modal-driver §6.
- [x] 10.3 3 unit tests in `ui::status_bar_tests` render the status bar directly: `modal_hint_cell_shows_primary_chords` (160-col happy path verifies `Space:toggle`, `Enter:confirm`, `Esc:cancel` + `modal: section-move` co-render and the refresh stamp is replaced); `modal_hint_cell_empty_when_no_modal` (no-modal fallback keeps the refresh stamp + `mode: normal`); `toast_outranks_modal_hints_in_center_cell` (toast wins). Five existing modal-active snapshots (`graph_create_filename_prompt`, `graph_move_target_banner`, `graph_rename_note_modal`, `graph_tab_preset_picker_open`, `graph_tab_search_picker_open`) re-blessed to show the new hint cell at 80 cols (the cell truncates gracefully when chords don't fit).

## 11. Documentation

- [x] 11.1 `docs/commands.md` authored: concepts (Command, CommandDef, scope, KeyMap, input pipeline), tools (`ft commands list`, `ft commands docs`, `ft do` with exit-code table), mixed-granularity model, `?` overlay generation, status-bar hint contract, recipes for adding a new command/modal/headless handler.
- [x] 11.2 `docs/architecture.md` gained a "Commands and keymaps (TUI)" section after the modal-driver write-up: enumerates the module map, summarises the input pipeline, documents the status-bar hint precedence, and points to `docs/commands.md` and `docs/keybindings.md`.
- [x] 11.3 README.md updated: the Interactive TUI section now introduces the Command/Keymap model and shows `ft commands list`, `ft commands docs`, and `ft do tasks.complete-by-id`; the Documentation list links to both `docs/commands.md` and `docs/keybindings.md`.

## 12. Build validation

- [x] 12.1 `cargo build --release` — clean
- [x] 12.2 `cargo test --workspace` — 770 binary tests + workspace tests pass. Snapshot re-blesses: 4 help overlays in §6, 1 capture-var date-rollover in §5, and 5 modal-active overlays in §10 (status-bar hint cell now renders).
