...
- [ ] 1.1 Add `ft_core::graph::drift` with the concept universe
...
- [ ] 2.1 Add the `drift` subcommand to `ft notes`: header rows
...
- [ ] 2.2 Integration tests: drifted fixture vault (report shape, ghost
  marker, merge suggestion), note↔note alias case, `--limit`/`--json`,
  clean-vault empty case
- [ ] 2.3 Round-trip sanity: run the suggested rename from the report
  against the fixture and assert the pair disappears from the next
  report

## 3. Docs + wrap-up

- [ ] 3.1 Document the drift report in `docs/guide/notes.md` (next to
  the Ghosts section) and cross-reference it from philosophy.md's
  "Keeping names honest" defense list
- [ ] 3.2 Update `openspec/explorations/note-flow-reframe.md` session 4
...
- [ ] 3.3 Full invariant sweep: `cargo build --release`,
