...
**Reason**: Absorbed into the `graph-model` capability. `NodeKind::Paragraph(ParagraphData)` still exists with the same payload (`source_file`, `line_start`, `line_end`, `text`); only its spec home moves.
**Migration**: See the `graph-model` capability spec — paragraph nodes are part of the unified contents model. `ParagraphData` is unchanged.

### Requirement: OwnsParagraph edges
**Reason**: Replaced by nearest-container `OwnsParagraph` (note OR heading → paragraph), not the prior flat note→paragraph model.
**Migration**: Use `graph_model::OwnsParagraph nearest-container ownership`. Consumers doing `outgoing(note).filter(OwnsParagraph)` must switch to `Graph::note_paragraphs(note)`.

### Requirement: ParagraphLink edges
**Reason**: Replaced by the unified `ParagraphLink(LinkEdge)` carrying full data (wiki + markdown forms, `is_embed`, byte_range), not the prior wiki-only data-less edge.
**Migration**: Use `graph_model::Unified link edge kinds` and `graph_model::Shared LinkEdge payload across link levels`.

### Requirement: Paragraph index
**Reason**: Absorbed into `graph-model` (the `paragraph_index` side table and `paragraph_by_loc` lookup are unchanged; only the spec home moves).
**Migration**: See `graph-model`. The lookup API is unchanged.

### Requirement: Paragraph extraction boundaries
**Reason**: Absorbed into `graph-model`. `markdown::extract_paragraphs` boundary rules are unchanged (blank lines, ATX headings, `--`/`---` rules; frontmatter/fenced/indented code skipped).
**Migration**: See `graph-model`. The heading line still begins a new paragraph and is absorbed into its `text` (Fork A2).

### Requirement: Graph build includes paragraph extraction
**Reason**: Absorbed into `graph-model` `Build phase ordering` requirement.
**Migration**: See `graph_model::Build phase ordering`.

### Requirement: refresh_note removes and re-inserts paragraph nodes
**Reason**: Absorbed and generalized into `graph_model::refresh_note re-inserts headings and all link kinds`.
**Migration**: See `graph_model::refresh_note re-inserts headings and all link kinds`.

### Requirement: Graph query DSL node-kind filter for paragraphs
**Reason**: Absorbed into `graph_model::Graph query DSL support for new node and edge kinds`.
**Migration**: See `graph_model::Graph query DSL support for new node and edge kinds`.

### Requirement: Graph query DSL edge-kind filters for new edges
**Reason**: Absorbed into `graph_model::Graph query DSL support for new node and edge kinds` (which adds `owns-heading`/`note-link`/`heading-link`/`paragraph-link` and the `edge.embed` predicate).
**Migration**: See `graph_model::Graph query DSL support for new node and edge kinds`.
