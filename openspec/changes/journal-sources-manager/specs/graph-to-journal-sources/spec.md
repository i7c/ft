...
### Requirement: Graph tab `Shift+A` adds selected nodes to Journal sources

The Graph tab SHALL provide a `graph.add-to-journal-sources` command (bound to `Shift+A` in the default keymap) that ships the active view's multi-selection (or the cursor row when the multi-selection is empty) to the Journal tab as additional sources.

#### Scenario: Multi-selection drives the target list
- **WHEN** the user has three rows multi-selected on the Graph tab (`Foo.md`, `Bar.md`, ghost `Phantom`) and presses `Shift+A`
...
- **WHEN** no rows are multi-selected and the cursor is on `Foo.md` and the user presses `Shift+A`
...
The Graph tab's `Tab::help_sections()` SHALL include a row for `Shift+A` describing the "append selected to Journal sources" action, in the same section that already documents `Shift+J`.

#### Scenario: Help overlay lists the chord
- **WHEN** the user presses `?` on the Graph tab
- **THEN** the overlay contains a row reading `Shift+A: append selected (or cursor) to Journal sources`
