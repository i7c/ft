...
- [x] 1.1 Add `mtime: std::time::SystemTime` and
      ...
      filesystem pass.
- [x] 1.2 Delete `scan::markdown_files_with_mtime` (no remaining
      callers after task 2). Expected: `Scan::files` is the only mtime
      source.
- [x] 1.3 Scan tests: distinct-mtime capture (write two files with
...
- [x] 2.1 `search::recent_hits(scan: &Scan, recents: &RecentsLog,
...
- [x] 3.1 `recent::build_recent(..., scan: &Scan)` gains the scan
      ...
      in the scan.
- [x] 3.2 Update `build_recent` callers (recent tab, cmd/notes.rs
      `run_recent`, ft-core tests) with a scan argument.
- [x] 3.3 Tests: (a) fixture with a synth note — excluded without a
...
- [x] 4.1 `pulse::compute_pulse(..., scan: &Scan)` gains the scan
...
- [x] 4.2 Update `compute_pulse` callers (pulse tab, gather tab's
...
- [x] 4.3 Tests: (a) synth-note callout exclusion works with a scan-fed
      gate; (b) fallback for a file created after the scan.

## 5. Journal aliases from the scan

- [x] 5.1 `gather::find_related_range(headings, total_lines: u32)` —
      drop the content parameter (callers pass the line count).
- [x] 5.2 `gather::resolve_related_aliases(graph, note_id, pf:
...
- [x] 5.3 `gather::build_gather(..., scan: &Scan)` gains the scan
...
- [x] 6.1 `search::fuzzy_find_from_scan(scan: &Scan, query, opts) ->
...
- [x] 6.2 TUI picker: `FilePickerSource` (and the empty-input source)
...
- [x] 6.3 Picker tests: scan-fed query matches vault-fed results for a
      fixed fixture; fallback path exercised without a snapshot.

## 7. CLI + TUI call-site sweep

- [x] 7.1 Bind and pass scans in `cmd/pulse.rs`, `cmd/synth.rs`,
...
- [x] 8.4 Equivalence test: `fuzzy_find_from_scan(scan, q, opts)` vs
      `fuzzy_find(vault, q, opts)` over the `tiny/` fixture produce the
      same hits.

## 9. Final verification

- [x] 9.1 Run all five build invariants; fix fallout.
- [x] 9.2 Grep-audit: no `build_recent`/`compute_pulse`/`build_gather`/
