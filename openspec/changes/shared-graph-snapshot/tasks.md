...
- [ ] 2.1 Replace `GraphTab.graph: Option<Graph>` reads with the ctx
      snapshot; add `seen_generation` and re-derive views
      (`restore_all_views`) on generation change in `on_focus` /
      `on_graph_ready`
- [ ] 2.2 Convert the ~12 mutation-site rebuilds (`scan(); Graph::build`)
      into `RefreshGraph` posts with pending cursor anchors (`NodeKey` /
      `(path, line)`) resolved in `on_graph_ready`
- [ ] 2.3 Render the loading placeholder when `ctx.snapshot` is `None`
- [ ] 2.4 Update GraphTab tests: insert `pump_graph_rebuild_for_test()` in
