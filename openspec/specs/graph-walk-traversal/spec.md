# graph-walk-traversal Specification

## Purpose
Define the termination and shape guarantees of `GraphQuery::walk`: node-level
dedup as the default visit policy, the reference-leaf marker on re-encountered
nodes, true-cycle handling, the depth bound, and the opt-in path-tree mode — so
an unbounded walk over any finite graph terminates in `O(V + E)`.
## Requirements
