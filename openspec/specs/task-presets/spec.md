# task-presets Specification

## Purpose
Storage of task query presets under `[tasks.presets]` (mirroring
`[graph.presets]`), with CLI/TUI resolution preferring user presets over
built-ins. Created by archiving change retag-modal-and-presets-namespace,
which moved the presets from the top-level `[presets]` section to the
`[tasks]` namespace.
