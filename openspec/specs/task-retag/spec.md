# task-retag Specification

## Purpose
A TUI shortcut on the Tasks tab that swaps one tag from a user-configured
short list into the selected task, replacing any prior tag from that list
while preserving all other tags. Backs the "switch context" workflow
(physical vs computer vs waiting) without the friction of the full edit
popup. Created by archiving change retag-modal-and-presets-namespace.
