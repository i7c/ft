...
Move-flow actions that need to read the host's in-memory graph or mutate view state SHALL be expressed as `AppRequest::GraphMove*` variants routed through `Tab::graph_move_*` hooks. The host's existing `confirm_target_from_tree`, `confirm_move_target`, and `execute_multi_move` helper methods SHALL stay on `GraphTab` and be called from the hook implementations.
...
- **THEN** it posts `AppRequest::OpenModalWithToast { modal, toast_text, toast_style }` rather than separate `Toast` + `OpenModal` requests — the single-slot `pending_request` would otherwise lose one of the two
